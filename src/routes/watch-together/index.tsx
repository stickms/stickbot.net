...
function LogIn() {
  ...
  const login = () => {
    ...
    createUser({ data: { username: inputRef.current.value } })
      .then((user) => {
        setId(user.id);
...
function RoomCreation() {
  ...
  const create = () => {
    ...
    createRoom({
      ...
      .then(({ roomid }) => navigate({ to: '/watch-together/room/$roomid', params: { roomid } }))
...
function RoomList() {
  ...
  return (
    <div className='flex flex-wrap w-200 max-w-[90vw] justify-center items-stretch gap-4'>
      {rooms.map((room) => (
        <Card key={room.id} className='min-w-56 min-h-56 flex flex-col items-start justify-between gap-4'>
          <div className='flex flex-col text-left gap-2'>
            <span>{room.name}</span>
            <span className='text-muted-foreground'>by: {room.owner.username}</span>
          </div>
          <div className='flex gap-2 ml-auto'>
            {room.owner.id === user.id && (
              <Button
                variant='destructive'
                onClick={() => {
                  deleteRoom({ data: { roomid: room.id } })
                    .then(() => setRooms(rooms.filter((rm) => room.id !== rm.id)));
                }}
              >
                Close
              </Button>
            )}
            <Button onClick={() => navigate({ to: '/watch-together/room/$roomid', params: { roomid: room.id } })}>
              Join
            </Button>
          </div>
        </Card>
